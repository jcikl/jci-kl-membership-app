...
const StandardEditModal: React.FC<StandardEditModalProps> = ({
    ...
    const loadStandardData = async () => {
      ...
      if (visible && initialValues) {
        ...
        if (initialValues.scoreSettings) {
          ...
          console.log('📊 分数设置数据已加载:', initialValues.scoreSettings.length, '项');
        } else {
          setScoreSettings([]);
        }
        ...
        try {
          ...
          if (standardData) {
            ...
            if (standardData.scoreSettings) {
              ...
              console.log('📊 分数设置数据已加载:', standardData.scoreSettings.length, '项');
            } else {
              setScoreSettings([]);
            }
  ...
  const handleSave = async () => {
    try {
      ...
      const dataToSave = {
        ...processedValues,
        scoreSettings: scoreSettings, // 保留原有结构用于兼容
            ...
            },
            {
              ...
              children: (
                <div>
                  ...
                  {/* 保留原有的分数设置（兼容性） */}
                  <Card title="传统分数设置（兼容模式）" size="small">
                    <div style={{ marginBottom: 16 }}>
                      <Space>
                        <Button 
                          type="default" 
