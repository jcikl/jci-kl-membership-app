...
export class EventProgramService {
  ...
  static async createEventPrograms(eventId: string, programs: EventProgram[]): Promise<void> {
    try {
      const batch = writeBatch(db);
      
      programs.forEach((program) => {
        const programRef = doc(collection(db, this.COLLECTION_NAME));
      ...
      console.error('批量创建活动程序失败:', error);
      throw new Error('批量创建活动程序失败');
    }
  }

  static async updateEventPrograms(_eventId: string, programs: EventProgram[]): Promise<void> {
    try {
      const batch = writeBatch(db);
      
      programs.forEach((program) => {
        if (program.id) {
          const programRef = doc(db, this.COLLECTION_NAME, program.id);
      ...
      console.error('批量更新活动程序失败:', error);
      throw new Error('批量更新活动程序失败');
    }
  }
}
...
export class EventCommitteeService {
  ...
  static async createEventCommittee(eventId: string, committee: CommitteeMember[]): Promise<void> {
    try {
      const batch = writeBatch(db);
      
      committee.forEach((member) => {
        const memberRef = doc(collection(db, this.COLLECTION_NAME));
        batch.set(memberRef, {
          ...member,
          eventId,
          createdAt: serverTimestamp(),
      ...
      console.error('批量创建委员会成员失败:', error);
      throw new Error('批量创建委员会成员失败');
    }
  }

  static async updateEventCommittee(_eventId: string, committee: CommitteeMember[]): Promise<void> {
    try {
      const batch = writeBatch(db);
      
      committee.forEach((member) => {
        if (member.id) {
          const memberRef = doc(db, this.COLLECTION_NAME, member.id);
      ...
      console.error('批量更新委员会成员失败:', error);
      throw new Error('批量更新委员会成员失败');
    }
  }
}
...
export class EventTrainerService {
  ...
  static async createEventTrainers(eventId: string, trainers: EventTrainer[]): Promise<void> {
    try {
      const batch = writeBatch(db);
      
      trainers.forEach((trainer) => {
        const trainerRef = doc(collection(db, this.COLLECTION_NAME));
      ...
      console.error('批量创建讲师失败:', error);
      throw new Error('批量创建讲师失败');
    }
  }

  static async updateEventTrainers(_eventId: string, trainers: EventTrainer[]): Promise<void> {
    try {
      const batch = writeBatch(db);
      
      trainers.forEach((trainer) => {
        if (trainer.id) {
          const trainerRef = doc(db, this.COLLECTION_NAME, trainer.id);
      ...
      console.error('批量更新讲师失败:', error);
      throw new Error('批量更新讲师失败');
    }
  }
}
