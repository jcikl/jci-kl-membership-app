...
export class FinancialReportGenerator {
    ...
    return {
      ...
      details: {
        ...
        transactions: transactions.map(t => ({
          ...
          category: t.transactionPurpose || '未分类'
        }))
      }
    };
  }
  ...
  async generateCashFlowStatement(fiscalYear: number): Promise<FinancialReportData> {
    ...
    return {
      ...
      summary: {
        operatingCashFlow,
        totalCashInflow: transactions.reduce((sum, t) => sum + t.income, 0),
        totalCashOutflow: transactions.reduce((sum, t) => sum + t.expense, 0),
  ...
  async generateProjectSummary(fiscalYear: number): Promise<FinancialReportData> {
    ...
    return {
      ...
      summary: {
        ...
        totalBudget: budgetData.reduce((sum, b) => sum + b.totalBudget, 0),
        totalSpent: budgetData.reduce((sum, b) => sum + b.spentAmount, 0),
      ...
      details: {
        ...
        transactions: transactions.map(t => ({
          ...
          amount: t.income > 0 ? t.income : -t.expense,
          type: t.income > 0 ? 'income' : 'expense',
          project: t.projectAccount || '未分类项目'
        }))
      }
    };
  }
  ...
  async generateBankReconciliation(fiscalYear: number): Promise<FinancialReportData> {
    ...
    return {
      ...
      summary: {
        ...
        totalOpeningBalance: bankAccounts.reduce((sum, a) => sum + a.initialAmount, 0),
        totalClosingBalance: bankAccounts.reduce((sum, a) => sum + a.currentBalance, 0),
        totalNetChange: bankAccounts.reduce((sum, a) => sum + (a.currentBalance - a.initialAmount), 0)
      },
      details: {
        reconciliationData
      }
  ...
  async generateReport(
    ...
    switch (reportType) {
      case 'income_statement':
        reportData = await generator.generateIncomeStatement(fiscalYear);
        break;
      case 'balance_sheet':
        reportData = await generator.generateBalanceSheet(fiscalYear);
        break;
      case 'cash_flow':
